...
class VueComponentCompiler {
	...
	*/
	private displayHelp(): void {
		...
		console.log(`\nUsage: ${this.embolden('vuecc')} input output [-h|--help|-v|--version]`);
    ...
    private compile(): void {
    	...
    	lr.eachLine(this.input, function(line: string, last: boolean) {
			...
			if (!in_method && !in_data && !in_services) {
				var m = line.match(/class\s(.*)\sextends\sVueController/);
			...
			if(last) {
				...
				function isNotBlankLine(line: string) {
					if (/^\s*\t*$/.test(line) == false)
						return true;
					return false;
				}

				lines = lines.filter(isNotBlankLine);
    ...
    */
	constructor(input: string, output: string, option: string, another: string, yao: string) {
		...
		if(this.verbose) this.printInfo(`Compiling Vue controller: ${this.embolden(input)}`);
		this.compile();
	}
}
